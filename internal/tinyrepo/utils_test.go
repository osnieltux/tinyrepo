...
// Every key the code and the panel's templates ask for must exist in the
// translation table.
//
// _t falls back to returning the key itself, which for keys written as English
// prose produces a page that looks translated and is not: a missing entry is
// invisible until somebody reads the panel in Spanish. This is what makes it
// visible.
func TestEveryTranslationKeyExists(t *testing.T) {
	...
	// _t("key") in the code, and {{t "key"}} in the panel's templates, which
	// live inside a Go string literal in admin_pages.go.
	patterns := []*regexp.Regexp{
		regexp.MustCompile(`_t\("([^"]+)"\)`),
		regexp.MustCompile(`\{\{\s*t\s+"([^"]+)"\s*\}\}`),
	}
	...
	for _, source := range sources {
		...
		for _, pattern := range patterns {
			for _, match := range pattern.FindAllStringSubmatch(string(body), -1) {
				key := match[1]
				if _, ok := messages["en"][key]; !ok {
					missing[key] = append(missing[key], source)
				}
			}
		}
	}

	for key, files := range missing {
		t.Errorf("no translation for %q, used in %s", key, strings.Join(files, ", "))
	}
}
