...
type Config struct {
	...
	Web struct {
		...
		// reach the mirror.
		BehindProxy bool
		// Config publishes /admin, the panel that edits config.toml and runs
		// the same actions the flags do. Off by default: it is the one part of
		// -ws that writes rather than reads, and it needs a login before it
		// will answer at all.
		Config bool
		// User and PasswordHash are the panel's single account. The hash is
		// produced by "tinyrepo -pw" and is a PBKDF2-SHA256 digest, never the
		// password itself.
		User         string
		PasswordHash string
...
func defaultConfig() Config {
	...
	// Off by default, and unusable until a password is set: the panel changes
	// what the machine downloads and where it writes it.
	cfg.Web.Config = false
	return cfg
}
...
func validateConfig(cfg *Config) error {
	...
	if cfg.Web.Config {
		// Refusing here rather than at the login page: a panel nobody can log
		// into is a misconfiguration, and finding out at startup beats finding
		// out from a browser.
		if _, _, err := authCredentials(cfg); err != nil {
			return fmt.Errorf("%s: [web].user / [web].passwordHash, %s (tinyrepo -pw)",
				_t("field"), _t("err auth missing"))
		}
	}
	if cfg.Proxy.Use {
		if cfg.Proxy.Host == "" {
