...
type repoHandler struct {
	...
	repoType string
	// admin is nil unless [web].config is on. It is answered before the
	// filesystem, so a directory called "admin" inside the repository cannot
	// shadow the panel, and hidden from listings so it is never mistaken for
...
func newHandler(config *Config, od *onDemand, stats *serverStats, panel *adminPanel) http.Handler {
	return repoHandler{
		admin:       panel,
...
func (h repoHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if h.admin != nil && isAdminPath(r.URL.Path) {
		h.admin.serve(w, r)
		return
	}
...
// isAdminPath reports whether a request belongs to the configuration panel.
// Exactly "/admin" or anything below it, so a file named "administration" in
// the repository is still served.
func isAdminPath(urlPath string) bool {
	return urlPath == adminPath || strings.HasPrefix(urlPath, adminPath+"/")
}
...
func serveRepo(config *Config, backend Backend, configPath string) error {
	root := config.Destination.Path
	...
	if config.Web.Config {
		...
		if err != nil {
			return fmt.Errorf("%s: %v", _t("err auth missing"), err)
		}

		// Loud, once, at startup: the panel writes config.toml and runs
		// downloads, and published to a network it is worth being told about
		// even though a password is required.
		if !isLoopback(config.Web.Listen) {
			logError(_t("warn panel exposed"), config.Web.Listen)
		}
		if !config.Web.BehindProxy {
			logError(_t("warn plain http"))
		}
	}
	...
	server := &http.Server{
		Handler:           newHandler(config, od, stats, panel),
	...
	if panel != nil {
		logError(_t("panel at"), webDisplayURL(listener.Addr())+strings.TrimPrefix(adminPath, "/")+"/")
	} else {
		// Said out loud rather than left silent: the panel being off looks
		// exactly like the panel being broken, and the difference is one line
		// in config.toml.
		logError(_t("panel off"))
	}
